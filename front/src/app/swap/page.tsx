...
function Swap() {
  ...
  const availableTickets: Ticket[] = [
    ...
    { id: 5, title: "Concert B Ticket", price: 55, imageUrl: "/api/placeholder/200/200" },
  ];
  ...
  const TicketCard = ({ ticket, isSelected, onSelect }: { ticket: Ticket, isSelected: boolean, onSelect: (ticket: Ticket) => void }) => (
    <div 
      className={`border rounded-lg p-4 flex flex-col items-center cursor-pointer ${isSelected ? 'border-blue-500 bg-blue-100' : ''}`}
      onClick={() => onSelect(ticket)}
    >
      <img src={ticket.imageUrl} alt={ticket.title} className="w-full h-40 object-cover mb-2 rounded" />
  ...
  return (
    <div className="container mx-auto mt-8 px-4">
      ...
      <div className="mt-8 flex justify-center">
        <button
          onClick={handleSwap}
          disabled={!isConnected || !selectedUserTicket || !selectedAvailableTicket}
